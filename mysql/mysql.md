...
学生和课程表是多对多的关系 通过选课表来实现 选课表内容（学生号，课程号）可为联合主键。
...
单个下划线通配符只匹配单个字符，如果要匹配多个字符，需要连续使用多个下划线通配符。例如，字符串“ab_”匹配以字符串“ab”开始长度为3的字符串，如abc、abp等等；字符串“a__d”(两道下斜杠)匹配在字符“a”和“d”之间包含两个字符的字符串，如"abcd"、"atud"等等。

```
select * from student where sname like 'zx__'（两道下斜杠）;
//查询姓名以zx开头且长度为4的记录
```
...
数据库的隔离性和一致性是一个需要开发者去权衡的问题，为数据库提供什么样的隔离性层级也就决定了数据库的性能以及可以达到什么样的一致性；在 SQL 标准中定义了四种数据库的事务的隔离级别：`READ UNCOMMITED`、`READ COMMITED`、`REPEATABLE READ` 和 `SERIALIZABLE`；每个事务的隔离级别其实都比上一级多解决了一个问题。
