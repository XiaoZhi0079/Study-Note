...
```
revoke delete on tlias from xiaozhi@localhost；//撤销用户xiaozhi对tlias数据库中所有表的删除权
```
...
InnoDB使用Buffer Pool（缓冲池）来提高mysql数据库的读写能力，缓存索引页，数据页，锁信息等等。

#### 核心原理

1. **加速数据访问**：数据库会把需要的数据页从磁盘加载到 Buffer Pool 中，这样后续操作就可以直接在内存中完成，而不用每次都去磁盘读取。
2. **延迟写入**：当数据被修改后，先把修改记录在 Buffer Pool 中的内存页，等到适当的时候再批量写回磁盘（类似“攒一堆作业再交”）。
3. **数据页管理**：Buffer Pool 会根据使用频率来决定哪些数据应该留在内存，哪些数据可以被替换。

#### 数据页生命周期

- 第一步

  ：当数据库需要访问数据时，首先检查 Buffer Pool 中是否已缓存了目标数据。
...
- Buffer Pool 内部通过链表结构管理内存块：
  1. **自由链表（Free List）：** 存储未使用的空闲内存块。
  2. **使用链表（LRU List）：** 存储当前正在使用或最近使用过的内存块。
  3. **脏页链表（Flush List）：** 存储所有需要刷入磁盘的脏页。

**数据页中的记录按照「主键」顺序组成单向链表**，单向链表的特点就是插入、删除非常方便，但是检索效率不高，最差的情况下需要遍历链表上的所有节点才能完成检索。

因此，数据页中有一个**页目录**，起到记录的索引作用，就像我们书那样，针对书中内容的每个章节设立了一个目录，想看某个章节的时候，可以查看目录，快速找到对应的章节的页数，而数据页中的页目录就是为了能快速找到记录。

Buffer Pool使用页做单位来缓存数据，默认大小为16K，可以调整大小。页之间使用链表连接，以实现逻辑上的连接，而不是物理上的连接。

为什么页之间不设计为物理上的连接？

```
因为随着数据的变化，页要不断的分裂，回收，重用，分裂出来的新页必须插到逻辑顺序里（比如在 100 和 101 之间）但磁盘上“紧挨着 100 的位置”可能：已经被别的页占了，或者属于别的索引/别的表空间区段，如若坚持使用物理分页，那么需要把后面一大段页整体搬家，消耗太多资源。
```
...
​        基于B+树的索引的本质是二分法，凭此可以快速查找范围或精准定位到某个值。而哈希索引是基于哈希算法，用哈希函数把“值空间”压缩成“地址空间”，只能精确定位到某个值。
...
MySQL 中 InnoDB 引擎的索引基于**B + 树**实现（MyISAM 也用 B + 树，仅细节不同），这是因为 B + 树相比二叉查找树、红黑树、B 树更适合磁盘存储的特性（磁盘 IO 是数据库性能的核心瓶颈），B+树实现索引的原理是二分法。

##### （1）B + 树的结构特点

B+树类似于一本书的目录，整棵B+树对应整个目录，节点中间节点相当于章节，小节，根节点则是最小的小节

![image-20260129231125763](./media/image-20260129231125763.png)

B + 树是一种**平衡多路查找树**，其结构分为**叶子节点**和**非叶子节点**：

- **非叶子节点**：仅存储**索引键值**（该区域内最大值最小值）和**指向子节点的指针**，不存储实际数据行，目的是让非叶子节点尽可能小，减少磁盘 IO 次数（一次 IO 可加载更多节点）。
- **叶子节点**
  - 存储**索引键值**和**实际数据行的指针 或**直接存储数据行（InnoDB 的聚簇索引）**。
  - 所有叶子节点通过**双向链表**连接，按索引键值有序排列，支持范围查询（如`WHERE id BETWEEN 10 AND 20`）。
- **平衡特性**：在索引的结构树中，每加一层，都会多一次磁盘的IO，二叉树每个节点只有两个分叉，所以一样的数据，二叉树层数很多，很高，而B+节点可以由多个分叉，这就使得层数大大降低。另外为了避免极端情况，比如热点数据在浅层，而冷数据在深层，查询冷数据的IO占用了大量的资源，拖垮整体进程，基于**最坏情况 + 可预测性 + 工程稳定性**三个要素的考虑，B + 树的所有叶子节点在同一层，保证查询的时间复杂度稳定在 O (log n)（n 为数据量）。
...
- 在 InnoDB 中：**主键索引 = 聚簇索引（通常情况）**
- 聚簇索引的记录值是主键，每张表一定会有聚簇索引。如若没有明确设置主键，则唯一非空值是主键，若无非空记录值，则隐式生成主键。
- 聚簇索引的叶子节点存放的是实际数据，所有完整的用户记录都存放在聚集索引的叶子节点；

##### 二级索引

- 二级索引的记录值是可以排序的值，比如年龄，身高这种自带排序犯法的值，对于字符串这些不好排序的，我们可以设置排序规则，类似与Java中自己手动实现的compare方法。
- 二级索引的叶子节点存放的是主键值，而不是实际数据。当innoDB引擎通过二级索引搜到主键时，**但是查询的数据不是或不只是主键值**，于是再通过聚簇索引再次查询获得完整数据行，称为**回表**。
...
//举例
CREATE INDEX idx_height ON user(height);
...
此UNIQUE的作用与加在属性上的UNIQUE（UNIQUE email VARCHAR(100)）的作用一样，属性值的唯一本身就是靠索引来保证的。

**创建索引的原则**

经常作为where条件过滤的字段考虑添加索引

注：索引字段涉及类型强转、mysql函数调用、表达式计算等,索引就无法起作用了。
...
//举例
DROP INDEX idx_height on user；
```

### Show索引

```
SHOW INDEX FROM dept\G;       //  \G竖排显示dept表的索引

*************************** 1. row ***************************
        Table: dept               
   Non_unique: 0                    //是否为唯一索引，0是非唯一，1是唯一
     Key_name: PRIMARY              //索引名
 Seq_in_index: 1                    //联合索引里的第几列
  Column_name: id                   //索引列
    Collation: A                    //排序方式 A为正排，B为倒排
  Cardinality: 5                    //该索引列大概有几种值
     Sub_part: NULL                 //是否为前缀索引
       Packed: NULL                  
         Null:                      //该值是否允许为空
   Index_type: BTREE                //索引类型：B族树索引，实际为B+树索引
      Comment:
Index_comment:
      Visible: YES                  //索引是否对优化器可见
*************************** 2. row ***************************
        Table: dept
   Non_unique: 0
     Key_name: name
...
慢 SQL 日志是 MySQL 服务端的一种**“把执行慢的语句落盘”**的诊断手段。它主要记录两类“值得关注”的 SQL：

1. **执行时间超过阈值**的语句（由 `long_query_time` 决定）
2. （可选）**没有使用索引**的语句（由 `log_queries_not_using_indexes` 决定）

它记录的核心目的不是“统计所有 SQL”，而是：

**把最值得优化的那部分 SQL 抓出来**（以服务端视角，包含执行耗时、锁等待、扫描行数等信息）。

### 慢日志的开关与存储方式

- `slow_query_log`：ON/OFF       //写入日志的开启与关闭
...
| Variable_name       | Value                                                             
+---------------------+----------------------------------------------------------------+
| slow_query_log_file | D:\phpstudy_pro\...DESKTOP-5RVJQEQ-slow.log |
+---------------------+----------------------------------------------------------------+
```
...
**脏读(Dirty Read)**: 一个事务读取了另一个事务**尚未提交的数据**，该问题只可能出现在 `READ UNCOMMITTED` 隔离级别下。
**不可重复读(NonRepeatable Read)**: 在同一个事务中，对**同一行记录**的两次读取，返回了不同的结果，原因是其他事务在期间**提交了 update 或 delete 操作**。
**虚读(Phantom Read)/幻读:** 在同一事务中，对**同一个范围条件**进行多次查询，后一次查询**读到了前一次不存在的“新行”**，原因是其他事务在期间**提交了 INSERT 操作**。
...
#### 表级锁
 ...
 对整张表加锁。开销小,加锁快,不会出现死锁;锁粒度大,发生锁冲突的概率高,并发度低。
...
#### 行级锁
 ...
 对某行记录加锁。开销大,加锁慢,会出现死锁;锁定粒度最小,发生锁冲突的概率最低,并发度高。
...
**写锁**/**排它锁**(Exclusive),又称为X 锁,写锁：我在改，别人既不能读，也不能改。

```
SELECT * FROM user WHERE id=1 FOR UPDATE;  // UPDATE / DELETE 默认就是 X 锁 forupdate是给                                                 select加锁的操作 意为查完马上改 但可以不改
```

**读锁**/**共享锁**(Shared),又称为S锁,读锁：我在读，别人也可以一起读，但谁都别改。
...
死锁：两个或多个事务互相持有对方需要的锁，形成环路，永远等不到。
 InnoDB 会自动检测死锁，并主动回滚其中一个事务。
