...
![image-20251113153906025](./media/image-20251113153906025.png)
...
​         需要排序的集合，其对象必须实现compare方法或者提供比较器
...
增强for循环底层是迭代器 但是迭代器是隐藏的 拿不到迭代器对象 所以无法解决索引导致的问题
...
![image-20251111160140460](./media/image-20251111160140460.png)

## Nginx

![image-20251111171412275](./media/image-20251111171412275.png)
...
使用${}做拼接字符串，mysql不会对语句进行预编译，如若${}中包含了"or 1=1"这样的语句，就会被当作语句执行，而#{}，相当于是使用 ? 符先1预编译了一次，然后把其中的值赋给了 ?，这样不管#{}其中写了任何的语句，都会当作值执行，而不会当作语句。 

除此之外，**任何出现在 SQL 里的“标识符”（表名、列名、函数名、排序/分组项）都不能来自用户自由输入。**

因为标识符无法像值一样，使用 ？占位执行**预编译**。

此时如何是好？方法如下：前端按传一个枚举 key，比如：`type=order` / `type=refund`后端用映射表决定真正的 SQL 模板。
